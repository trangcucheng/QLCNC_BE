...
export class RolesController {
  ...
  async getAllRoles(@Query() query: GetAllRolesDTO): Promise<VaiTro[]> {
    const { page = '1', pageSize = '10', orderBy } = query;

    // Xử lý orderBy string (vd: 'ngayTao:desc')
    let orderByObj: Prisma.VaiTroOrderByWithRelationInput | undefined;
    if (orderBy) {
      orderByObj = {
  ...
  @Get('/detail-role')
  async getRoleById(@Param('roleId') roleId: string): Promise<VaiTro | null> {
