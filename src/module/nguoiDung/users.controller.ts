...
export class UsersController {
  ...
  async getAllUsers(@Query() query: GetAllUsersDto): Promise<NguoiDung[]> {
    ...
    let orderByObj: Prisma.NguoiDungOrderByWithRelationInput | undefined;
    if (orderBy) {
    ...
    return this.userService.getAllUsers({
      ...
      orderBy: orderByObj,
      role: role, // truyền role riêng, không gán vào where
    });
  }
  ...
  async blockUser(
    ...
    try {
      ...
      return {
        status: 'success',
        statusCode: 200,
        message: `User with id ${userId} is now ${!updatedUser.trangThaiHoatDong ? 'blocked' : 'unblocked'}`,
      };
    } catch (error) {
      if (
        error instanceof Prisma.PrismaClientKnownRequestError &&
