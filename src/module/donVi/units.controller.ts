...
import { Prisma, DonVi } from '@prisma/client';
...
export class UnitsController {
  ...
  async getAllUnits(@Query() query: GetAllUnitsDTO): Promise<DonVi[]> {
    const { page = '1', pageSize = '10', orderBy } = query;

    // Xử lý orderBy string (vd: 'ngayTao:desc')
    let orderByObj: Prisma.DonViOrderByWithRelationInput | undefined;
    if (orderBy) {
      orderByObj = {
  ...
  async createUnit(@Body() createUnitDTO: CreateUnitDTO): Promise<DonVi> {
  ...
  async deleteUnit(@Query('unitId') unitId: string) {
    if (!unitId) {
      throw new BadRequestException('Unit ID is required');
    }
    try {
  ...
  @Get('/detail-unit')
  @Permissions('VIEW_UNIT')
  async getUnitById(@Query('unitId') unitId: string): Promise<DonVi | null> {
    if (!unitId) {
      throw new BadRequestException('Unit ID is required');
    }
